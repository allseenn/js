...
Движки написаны на C++, интерпретируют JS-скрипты, встроены в прикладное ПО:

- Браузеры
- Node.js, Electron, React Native, Deno
- ПО для встраиваемых систем (роботы, Falcon 9, Iskra)

Прикладное ПО работает в системных средах:
...
Коллбеки resolve и reject принимают максимум по одному аргументу (как правило объект). Все кроме первого аргумента игнорируется.

Промис возвращает что-то одно - результат resolve, результат reject, или undefined.
...
Коллбеки принимает только по одному параметру, поэтому можно не указывать аргументы в скобках:
...
Если параметров несколько, то они все в круглых скобках:

```js
promise.then(
    (result1, result2) => {console.log(result1, result2);}
)
```
