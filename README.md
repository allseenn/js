...
Один из механизмов асинхронности это передача в функцию, которая должна что-то ждать, в качестве аргумента функции обратного вызова. Обратный вызов выполнится когда процесс ожидания закончится.

```js
setTimeout(() => console.log('hello'), 1000);
```

Выполнение множества асинхронных дейстий в одну строку (инструкцию) означает передачу результата функций по цепочке.

Например, сравнение двух строк:
...
Уровень вложенности setTimeout() и других асихнронных фукнций может быть горомадным (сallback hell или pyramid of doom) для описания асинхронной программы. Для того чтобы упростить написание кода и ускорить процесс его выполнения, была добавлена возможность записывать вышеприведенный код более лаконично с помощью промисов.
