...
**Бинарные** операторы обрамляемые пробелами с двух сторон:
...
**Тернарный** оператор обрамляется пробелами, чтобы не спутать его с опциональной цепочкой

- `a ? b : c` - Тернарный условный оператор

**Особый** (отдельный вид) оператор опциональный цепочки не обрамляется пробелами:
...
При цепочке логических операторов || вычисление идет слева направо и переменной присвоится первое значение, которое не `false`.
...
Слева направо переменной присвоится последнее значение, которое не `true`.
...
Как правило, типы данных в js назначаются динамически при создании переменных, они не указываются явно. Но, типы можно переопределить:
...
Арифметические операции кроме бинарного сложения всегда приводят типы к одному:
...
В случае унарного плюса (минуса), получим число:
...
У объекта браузера window есть методы ввода:
...
Является синхронным блокирующим основной поток методом, который отображает диалоговое окно с сообщением в нем и ожидает действия пользователя.

```js
alert("Hello");
```

Поддерживает только один аргумент, нельзя указать несколько через запятую как в console.log.
Любое выражение в скобках будет вычислено, а результат выведен в диалоговое окно.

#### prompt

Выводит диалоговое окно с полем ввода и кнопкой ОК. При нажатии ОК возвращается введенное значение, в противном случае возвращается null.

```js
let name = prompt("What is your name?", "Roma");
console.log(name);
```

Принимает два аргумента:

1. обязательный - текст вводимый в диалоговом окне
2. необязательный - значение по умолчанию

#### confirm

Выводит диалоговое окно с двумя кнопками ОК и Отмена. При нажатии ОК возвращается true, в противном случае false.
...
#### Методы ввода для node.js

Для серверной среды такие методы не предусмотрены, но их можно реализовать с помощью сторонних пакетов, либо написать самому:
...
В данном случае все работает в одном блоке, и каждый case не нужно оборачивать в фигурные скобки. Но, есть случае когда это нужно делать, например когда каждый case будет иметь свои переменные с одинаковыми именами:

```js
let a = 10;
switch (a) {
    case 10: {
        const message = 'Десять';
        console.log(message);
        break;
    }
    case 20: {
        const message = 'Двадцать';
        console.log(message);
        break;
    }
    default:
...
Ключевое слово `break` пишут чтобы прервать выполнение switch в нужном case, если этого не делать, то будет выполняться все case.
В default не нужно писать break, т.к. он выполнятся в конце и за ним нет больше блоков.
...
Функции должны именоваться так: действие (глагола) над чем (существительное) в снейк-кейсе:
...
**Параметр функции** - это имя переменной, объявленное как с ключевым словом let, которое находится внутри скобок после имени функции.
...
Если опциональных параметров несколько со своими значениями по умолчанию и мы хотим передать только одно значение аргумента для последнего параметра. Тогда нужно передать в качестве первых значений undefined:

```js
function minus(a = 10, b = 10) {
    return a - b;
}

console.log(minus(undefined, 5)); // 5
```
...
- легко переиспользуемые в других местах или коде
...
Ключевое слово return возвращает результат, который идет правее него, без правой части будет возвращаться **undefined**:
...
Возвращаемый результат может быть любого типа данных: строка, число, объект, массив, функция, null, undefined и т.д:
...
Такие функции могут быть переопределены (перезаписаны), но не могут быть переопределены внутри тела функции.

Декларативные функции содержат псевдомассив **arguments**, в котором хранятся переданные в переданные аргументы.

Содержат **this** (контекст вызова), который указывает на объект в котором была создана функция.
...
Функциональное выражение не обладает всплытием, т.е. вызывать функции нельзя до ее определения.

Функциональное выражение в отличие от декларативной функции нельзя переопределить, если она присваивается константе, но можно изменить если вместо const поставить let:
...
Такие функции не обладают всплытием, т.е. вызывать функции нельзя до ее определения.

Они не имеют доступа к служебному псевдомассиву arguments.

Стрелочная функция не имеет своего контекста, она использует родительский, поэтому this берется из родительского контекста.

##### Стрелочная функция с одним параметром и без return

Если стрелочная функция имеет один параметр, то можно убрать скобки, а если она в одну строку, то можно не указывать return:
...
Объявить (написать) функцию не достаточно, ее нужно вызвать (запустить), это делается с помощью следующей инструкции со скобками:

```js
myFunction()
```

Без скобок, функцию по имени можно передавать в качестве параметра другой функции:

```js
console.log(myFunction); // в Chrome выведет объявление функции, в других средах объект функции 
```

Функции при передачи в другие функции в качестве параметров в основной фукнции могут использвоться как call-back
...
mainFunc(()=>console.log('callback'))
```

При вызове функции создается для нее объект, содержащий все ее параметры и локальные переменные, который называется контекстом.

### Стек вызовов

ОЗУ в JS делится на 3 вида памяти:

- Стековая память (stack memory) - специализированная область памяти (структура данных), предназначенная для хранения стека вызовов
...
Очень удобен для бесконечных циклов: `while (true)`
...
Объект можно создать 3 способами:

- С помощью конструктора Object: `const obj = new Object();`
- С помощью литеральной нотации (литерал объекта): `const obj = {};`
- С помощью метода `const objWithoutProto = Object.create(null)` создается объект без прототипа

Объекты имеют свойства (характеристики, поля) и методы (действия, интерфейсы).
...
Свойства объектов это набор пар ключ-значение, которые находятся в фигурных скобках.
...
К свойствам можно обратиться двумя способами:
...
С помощью точечной нотации нельзя обратиться к свойствам, которое целиком состоят из цифр, имеют пробелы или дефис в именовании:
...
Именами свойств могут быть даже зарезервированные слова, что не позволительно для именования обычных переменных:

```js
const obj = {
  'if': 'value',
  'else': 'another value',
  'while': 'yet another value'
};
```
...
Имена и их значения могут быть вычисляемыми:

```js
propName = console.log('name');
propValue = console.log('John');
...
Чтобы вычислялось имя свойства, его значение должно быть добавлено в скобки, куда можно напрямую записать сложное выражение для вычисления и присвоения результата имени свойства:
...
Данный оператор можно заменить проверкой типа сравнения:

```js
console.log({left: 1, right: 2}.left !== undefined); // → true
```

Но, есть косяк если само значение будет равно undefined:

```js
console.log({left: undefined, right: 2}.left !== undefined); // → false
```

###### for...in

Для итерации по объекту можно использовать цикл **for** c оператором **in**:
...
По умолчанию цикл for...in выводит свойства объекта в том порядке, в каком они записаны изначально. Но, если свойства это цифры, либо типы которые можно привести к цифрам, то они будут сортироваться в порядке возрастания:
...
console.log(nums); // {0: "zero", 1: "one", 2: "two", 3: "three", 4: "four", 5: "five", 6: "six"}
```

###### Перечисляемые свойства объекта

Это свойства которые перечисляются в цикле **for...in**:
...
С помощью цикла выше будут выведены все собственные свойства объекта и 2 неперечисляемых свойства: **__proto__** и **constructor**.

Чтобы вывести только собственные свойства используется **Object.hasOwnProperty**:
...
В IDE VSCode при отладке в панели **Variables** используется особая подсветка:

- перечисляемые свойства в ярком фиолетовом цвете
- неперечисляемые свойства в темном фиолетовом цвете

В JavaScript, свойства, которые не перечисляются с помощью цикла for...in, могут быть скрытыми системными свойствами или свойствами, которые были добавлены через определенные методы или функции.

Некоторые примеры скрытых системных свойств, которые не будут перечисляться с помощью for...in, включают:

- Свойства объекта prototype и constructor
- Свойства объекта Symbol (например, Symbol.iterator)
- Свойства, добавленные через методы, такие как Object.defineProperty или Object.defineProperties
...
Имена методов, как и свойства могут быть заключены в кавычки, осовбенно если в имени есть пробел или спецсимволы. Обращаться или вызывать методы можно с помощью точечной нотации или квадратных скобок.
...
Переменные непримитивных типов данных хранят указатель на ячейку в памяти (ссылку), поэтому объекты сравниваются по ссылке:
...
Для одноуровневых объектов можно использовать перебор и сравнение свойств:

```js
const user1 = { name: "Иван", age: 30 };
const user2 = { name: "Иван", age: 30, sex: "male" };

function compare(obj1, obj2){
    if(Object.keys(obj1).length !== Object(obj2).length) return false
    for(let key in obj1) if(obj1[key] !== obj2[key]) return false
    return true
}

console.log(compare(user1, user2)); // false
```
...
Генератор - это специальная функция со звездочкой (function*), которая возвращает объект итератора, который будет возвращать значения внутри, позволяя приостанавливать и продолжать работу, принимая промежуточные данные и продолжая выполнение с новыми вводными.
...
let it = generator(); // не вызывает генератор, а возвращает объект
...
В отличие от функции, вызов генератора его не запускает. Это можно сделать с помощью метода next.
После запуска выполенния генератора происходит до ключевого слова yield, которое возвращает объект вида `{value: any, done: boolean}`.
При этом внутренне состояине генератора сохраняется. Поэтому следующий вызов next() начнет выполеннения с этого состояния и до тех пор пока снова не встретится yield, return или done true.
Генераторы можно итеррировать с помощью цикла for...of:
...
Передавать данные в генератор можно как параметр функции next():
...
it.next();   
// a = 'Привет'   
it.next('Привет'); // { value: 1, done: false }
 // b = 'Мир'
it.next('Мир'); // { value: 1, done: false }
 // c = '!', выведет: "Привет Мир !"
it.next('!');  // { value: 1, done: false }
// Привет Мир !
```

Тут yeild возвращает значение { value: 1, done: false }, но принимает от next() параметр, который присваивается внутренней переменной.

Пример с диапазоном из прошлого пункта про итераторы, но с генератором:
...
Асинхронными делают функции, которые чем-то могут быть заняты, заблокировав основной поток, например:

- доступ к БД
- чение или запись файлов
- отправка или получание данных по сети

В вебе часто код для подгрузки картинок делают асинхронным, чтобы пользователю не дожидаясь были доступны текст и элементы управления.

Встроенные функции и методы являются либо синхронными, либо асинхронными по своей природе. Циклы `for`, `while` — синхронны, но код внутри них можно организовать асинхронно.

Интерпретатор JS различает синхронные и асинхронная функции, которые имеют свой поток выполнения:

Синхроная функция:

1. попадает в стек вызовов
2. ждет своего выполнения
3. выполняется когда подойдет их время

Асинхронная функция:

1. попадет в стек, где выполняется ее синхронная часть, например создается объект промиса
2. вместе с callback передается в нужнный процессор API на выполененние
3. после выполенния асинхронной функции в API, ее колбэк передается в одну из очередей (макроочередь, микроочередь)
4. после освобождения стека вызовов колбеки перемещаются из очередей в него на выполененние
5. выполняется коллбек, когда подойдет его время
...
БРАУЗЕР
    ...
    └── СРЕДА:
        ...
        └──МЕХАНИЗМЫ
            └── **Event Loop**
            └── **Callback Queue (Macrotask Queues)**
            └── **Microtask Queue**
...
Интерпретатор JS (interpreter) движка (см. [Введение](#intro)) при чтении кода определяет (по имени, контексту и т.д.) какие функции:

1. синхронные - помещаются **call stack**
2. асинхронные - помещаются в WEB API
   - коллбеки асинхронных функций помещаются в очереди специализированных движков или процессоров:
     - **microtask queue**
     - **macrotask queues** (callback queue):
       - **Rendering queue**
       - **DOM queue**
       - **CSS queue**
       - **Audio queue**
...
Каждый отдельно взятый движок или процессор (JS, DOM, CSS) выполняет свои задачи синхронно в своих очередях, за всеми очередями следит цикл событий.

### Event Loop

**Цикл Событий** (Event Loop) - механизм среды (браузер или node).

Цикл событий повторяется каждые ~16.7 мс (1000/60), где 60 это частота обновления экрана монитора, на смартфонах будет около 200fps, т.е. 5 мс.

Event Loop выполняет переносы из очередей по строгому алгоритму:

1. Проверяет Call Stack - если не пуст, ждет пока движок JS завершит выполнение всех задач
2. Когда Call Stack пуст:
   - Шаг 1: Берет ВСЕ готовые задачи из Microtask Queue и переносит в Call Stack
   - Шаг 2: Если Microtask Queue пуста, берет ОДНУ задачу из Macrotask Queue и переносит в Call Stack
   - Шаг 3: Проверяет необходимость рендеринга (в браузере) - дает сигнал рендеринг-движку
   - Шаг 4: Повторяет цикл
...
1. Первая строка синхроная функция, она попадает в Call Stack.
   - Т.к. функция единственная в Call Stack, она выполняется JS-движком и в консоле видим "Start algorithm!"
2. Вторая строка setTimeout() это асинхронная фукнция, после того как она попала в стек, выполняет синхронную часть при наличии
   - setTimeout() дождавшись своей очереди идет на выполенине API браузера.
3. Асинхронная функция уйдя в API не блокирет поток, поэтому в стек попадает 5 строка с console.log
   - Console.log исполняется выводя "End synchronous code."
4. Находясь в WEB API setTimeout() ожидает 5 секунд
   - Через 5 секунд setTimeout() исполняется и возвращает в макротаск-очередь свой коллбек console.log
5. Работа Event Loop
   - ожидает освобождение стека
   - ожидает освобождение очереди микротасков
   - помещает коллбек в Call Stack
6. Происходит выполенние коллбека и в консоле видим "This will be printed after 5 seconds!"

Как видно из примера выше асинхронные операции не выполняются синхронно, т.е. последовательно. В коде сначала в любой случае выполнятся все синхронные фукнции, даже если setTimeout() выставлен в 0.

Асинхронный код "заразен" (contagious), как только он появится в коде, весь остальной код нужно тоже делать асинхронным.

Поэтому для последоваетльной работы с асинхронным кодом нужно делать код вложенным.

### Подходы ассинхронности

<img src='./img/async_compare.png'>
...
Один из подходов асинхронности это передача в функцию, которая должна что-то ждать, в качестве аргумента функции обратного вызова. Обратный вызов выполнится когда процесс ожидания закончится.

```js
setTimeout(() => console.log('hello'), 1000);
```

Выполнение нескольких асинхронных действий предполагает, что последующее действие (функция) будет являтся колбоком родительской.

Например, сравнение содержимого двух файлов:
...
В примере выше использована абстрактная (несуществующая) функция readTextFile() для чтения текстовых файлов.
Первый вызов readTextFile() содержит колбек, который использует второй вызов readTextFile(), который в свою очередь использует колбек для сравнения содержимого двух ранее считанных файлов.

#### setTimout(), setInterval()

Данный функции помещают колбеки в очерь отоложенных задач и используются в современных подходах асинхронности.
...
Заразность, при использовании асинхронных операций последующий код в конкретной логике должен быть также асинхронным. В противном случае, при последующих синхронных операциях, они выполняться раньше, чем асинхронные.
Для соблюдения последовательности используют вложенность.

Напрмер сэмулируем получение данных сервера с задержками:

```js
console.log('Start download');
setTimeout(() => {
    console.log("Downloading");
    const data = {id: 1, status: 'working'};
    setTimeout(() => {
...
Уровень вложенности setTimeout() и других асихнронных фукнций может быть горомадным (сallback hell или pyramid of doom) для описания асинхронной программы. Выполенение сложных асинхронных задач типа циклов, будет крайне неудобно.

Для того чтобы упростить написание кода и ускорить процесс его выполнения, была добавлена возможность записывать вышеприведенный код более лаконично с помощью промисов.

### 2. Promise

Для борьбы с большой вложенностиью при обратных вызовах были изобретены промисы, которые позволяют выполнять асихнронные операции по цепочке. Что улучшает читаемость кода.
...
Промис можно представить как некое устойство, которое позволяет нам отслеживать состояние асинхронных операций и не задаваться вопросом когда поступят нужние значения.

Идея промисов возникла еще в 70-80-х годах в теории языков программирования, но в JavaScript они появились ES6 (2016).

Конструктор промиса принимает функцию-исполнитель (executor), являющаюся колбэком, вызывается в момент создания промиса.
...
Функция-исполнтиель принимает в качестве аргументов другие две внутренни функции состояний промиса:
...
Функции состояний меняют внутренних слоты объекта Promise:
...
Данные слоты можно увидеть в браузере chrome введя в консоли либо `promise` либо `console.log(promise)`:
...
При определении функции-исполнителя в ее тело можно записать любой код включай синхронный, например для вывода сообщений, но не рекомендуется, т.к. тяжелый код может подвесить **event loop**.

Создадим промис с задержкой с помощью функции таймаута:
...
Программисты редко создают собственные промисы, они как правило пользуются уже существующими, следующие сущности возращают промисы:
...
**Методы промисов делятся на**:

1. Прототипные (привязанные к прототипу Promise)
   - then
   - catch
   - finally
2. Статические (привязанные к классу Promise)
   - resolve
   - reject
   - all
   - allSettled
...
Колбеки-обработчики (стрелочные фукнции) нужны для того, чтобы совершить действие по результату промиса. Данной действие нельзя выполнить в синхронной операции.
...
Вложенный асинхронный код с setTimeout из конца позапрошлого [пункта](#event-loop)  можно переписать с помощью промисов:
...
const promise = new Promise((resolve, reject) => {
   setTimeout(() => {
       console.log("Downloading");
       const data = {id: 1, status: 'working'};
       resolve(data); // передаем полученные данные наружу
   }, 2000) 
})

promise.then(data => { // получаем переданные ранее данные из промиса
    const promise = new Promise((resolve, reject) => {
        setTimeout(() => {
            console.log("Download complete");
            data.status = 'completed';
...
Если обработчик возвращает также объект промис, то для него можно вызвать then (и другие прототипные методы) и так по цепочке:
...
- в конструктор первого промиса передали стрелочную функцию исполнитель, т.к. у нее один аргумент `resolve` то он не в скобках.
- стрелочная фукнция возвращает строку с setTimeout, поэтому можно не указывать ключевое слово return и не надо скобок.
- первый then в случае выполенния первого промиса передает его результат плюс 1 в конструктор второго промиса
- второй then возвращает промис с результатом второго промиса плюс 1
- третий then возвращает функцию-колбэк, которым выступает `console.log`, т.к. параметров не указано в `console.log` передается result

Код загрузки данных из прошлого пункта с помощью цепочки промисов можно переписать так:
...
}).then(data => { // получаем переданные ранее данные из промиса
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            console.log("Download complete");
            data.status = 'completed';
...
// в цепочке вызвово можно использовать синхронный код для работы с данными без созадиня еще промисов
```

###### Thenable

В цепочке вместо создания промисов можно использовать класс Thenable:
...
Это не замена возратов промисов - это альтернатива

Подход при котором асинхронные функции возвращают объект Promise, который представляет будущий результат. Его форма более походит на синхронные програмы.
Данный объект имеет метод then, который позволяет зарегистрировать функцию вызываемую при успешном завершении асинхронной операции.
Когда промис разрешен, т.е. появилось значение, then выполняется со значением от прошлого промиса.

Наипростейший способ вызова промиса это:
...
Данный промис выполняется сразу, т.к. он получает переданное ему значение, которое сразу возвращает.

Если же нам нужен не моментальный промис, то можем использовать конструктор:
...
Метод then возвращает промис, который можно передать по цепочке из then. А также работать над разрезолвлеными значениями каждого промиса:
...
Выше представлена функция вывода случайного текстового файла из списка:

1. Первая строка возвращает промис со списком текстовых файлов
2. Вторая строка возвращает промис с перечнем текстовых файлов
3. Возвращает промис со случайным текстовым файлом
4. Последняя строка возвращает промис со содержимым текстового файла

Промис можно представить в виде некого устройства, которое позволяет не вдаваться в подробности, когда будет получен результат.

##### catch

В прмерах ранеее в then почти не используется второй коллбек-обработчик `onRejected`, т.к. в новом синтаксите вместо него можно использовать метод catch:
...
это тоже самое что и then с коллбеком `onRejected`:
...
Можно сразу создать отклоненный, либо успешный промис:

```js
const prom = Promise.reject('test');

prom.catch(error => {
    console.log('Ошибка:', error); // Выведет: Ошибка: test
});
```

В цепочке промисов достаточно одного вызова catch, что бы отловить ошибку из любого промиса в цепочке.

catch как и then возвращает промис, который в случае успеха содержит результат и отклоненние в случае ошибки.

Ошибки можно пробрасывать далее минуя последуюшие then:

```js
new Promise((resolve, reject) => {
    throw new Error('Error');
}).catch(error => {
    if(error instanceof URIEroor) {
        // обработка ошибки
    } else {
        throw error;
    }
...
Если ошибка не будет обработана, то в среде браузера вывалится собственная ошибка.

###### error‑first callback

До появления промисов существовало общее соглашение для асинхронного кода, что первым аргументом колбэка всегда передаётся объект ошибки (Error). Если операция завершилась успешно, этот аргумент равен null или undefined.

Второй и последующие аргументы содержат успешные результаты:
...
Сегодня этот паттерн устарел, его вытеснили Promise и async/await, но он всё ещё встречается в старом коде и некоторых API Node.js.

##### finally

Данный метод ES2018 позволяет выполнить код в конце цепочки промисов, независимо от состояния промиса и ошибок:
...
Метод не принимает никаких аргументов и не возвращает никаких результатов, он не знает что произошло ранее он выполняется при любом раскладе.

#### Статические методы промисов

Методы которые не принадлежат экземплярам (объектам) на основе Promise, а относятся к самому классу Promise.
Данные методы позволяют выполнить переденные в них в качестве параметров асинхронные операции параллельно.
...
Данные методы возвращают массив результатов, где через метод then можно применить коллбеки-обработчики и получить результаты асинхронных операций, в случае ошибки возвращается ее подкласс, типа AggregateError.

##### Promise.all()

Вернет массив с результатами работы всех промисов, если они все завершиилсь успешно.
...
Метод ES2020. Возвращает массив с результатами работы всех промисов, не важно они завершены успешно или нет.
...
Возвращает первый завершившийся промис с любым результатом (даже неуспешным):
...
```js
const promise = Promise.reject(new Error('error')).catch(console.log); // Error: error
```
...
```js
const promise = Promise.resolve(1).then(console.log); // 1
```
...
Asynchronous JavaScript And XML (AJAX) - подход, который позволяет обновлять страницу без перезагрузки: чаты, мессенджеры, уведомления и т.д.

Данный подход можно реализовать старым и новым способом:
...
Появился в ES5 2009 года, в связи с тем, что возникло большое количество ошибок в реализации JS. Помимо ошибок, было много предложений по улучшению языка, но из-за обратной совместимости не было реализовано. `use strict` включает не только строгий режим в плане исключения ошибок, но и включает поддержку новых возможностей.
...
[Визуальный тренажер](https://regex101.com/)

Regular expressions - это объекты класса RegExp, предназначенные для поиска и замены символов в строках по определенному шаблону.
...
```js
let re1 = new RegExp("abc", i); // i - case-insensitive
let re2 = new RegExp("\\dabc", g); // g - global
```
...
| -------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| d        | inDices                | используется с exec, добавляет к результату массив с начальным и конечным индексами совпадения                                |
...
| v        | Vnicode                | для нелатинских и латинских символов вместе                                                                                                                          |
...
Данный сокращения в JS работают только для ASCII латиницы:
...
Для символов в другой (не латинской) кодировки используется другой синтаксис

##### АСCII НЕлатиница

- `\p{L}` Любая буква (letter) - сокращение от `\p{General_Category=Letter}`
- `\p{N}` Любое число (number) - сокращение от `\p{General_Category=Number}`
- `\p{P}` Любой знак препинания (punctuation) - сокращение от `\p{General_Category=Punctuation}`
- `\P{L}` Любоая не буква (большая P инвертирует) - сокращение от `\P{General_Category=Letter}`
- `\p{Script=Hangul}` Любая буква китайского языка

##### Для Unicode письменностей

- `\p{Script=Hangul}\u` Любая буква китайского языка в кодировке Unicode

где `u` - это кодировка Unicode

В питоне чтобы использовать бодобный синтаксис нужно использовать сторонюю библиотеку `regex`:

```python
import regex  # Не стандартный re!
pattern = regex.compile(r'\p{Script=Hangul}')
```

##### Unicode числа

Не арабские числа в различных письменностях не работают с классом Number, т.е. не будут вычисляться

```js
console.log(Number("١٢٣"));    // NaN (арабско-индийские цифры)
console.log(Number("⅔"));      // NaN (дробь)
console.log(Number("²"));      // NaN (верхний индекс)

// А это сработает:
console.log(Number("123"));    // 123 (только арабские цифры)
```

Если нужна совместимость с числами JS, то лучше использвать символьный класс `\d`, который не найдет юникод числа не арабского происхождения, но те что найдет можно будет обработать как числа, т.е. произвести над ними арифметические действия:

```js
let text = "I have ５ apples and ٣ oranges"; // Использует полноширинные арабские-индийские цифры
let unicodeNumbers = text.match(/\p{Number}/gu); // ["５", "٣"] (тут пятерка похожа на обычную, но это не так, она не будет вычислена как число)
// Number(unicodeNumbers[0]) → NaN!

let standardNumbers = text.match(/\d/g); // [] - пусто, но безопасно
```
...
Для поиска по строком в многострочном тексте нужно использовать флаг m (multiline)
...
Теперь матчится первая строка, а не весь текст, а именно индекс 6, т.е. начало первого слова line.
...
Можно решить с помощью окружающих проверок **lookaround**:
...
- `(?<!\p{L})` - группа символов c негативный ретроспективной проверкой: ищем букву перед которой нет символа
- `(?!\p{L})` - группа символов c негативрой опережающие проверкой (lookahead): ищем букву за которой нет символа

### Окружающая провека

Lookaround - механизм проверки символа в зависимости от впереди или позадиидущих символов:

- Позитивный lookahead: (?=pattern)
- Негативный lookahead: (?!pattern)
- Позитивный lookbehind: (?<=pattern)
- Негативный lookbehind: (?<!pattern)
...
`?!` Впередиидущий не равен ...

После `a` впередиидущий не равен `пробел`:
...
Lookbehind - Регулярное выражение использует ретроспективную проверку, чтобы найти X, перед которым следует Y.

##### Позитивный lookbehind

`?<=` Ищем X если перед ним есть Y

Ищем `a` если перед ним есть `e`:
...
`?<!` Ищем X если перед ним НЕТ Y

Ищем `a` если перед ним НЕТ `пробела`:

```js
/a(?<! )/.exec("ab")  // Найдет "a" (перед "a" нет пробела)
```
...
Ищем совпадения бинарного числа с `b` на конце, либо шестнадцатеричного числа с `h` на конце, либо десятичного числа без постфикса.

```js
/^([01]+b|[\da-f]+h|\d+)$/.test("103")
```

Данный шаблон имеет 3 ветки (одна на систему счисления). Первая ветка ищет бинарное число, если даже заведомо передано десятичное, например 103.
...
Если несколько веток совпадают с тестовой строкой, то сравнивание заканчивается на первом успехе.

Механизм отката существует и у метасимволах повторения (+, *):
...
Чтобы добиться замены всех вхождений (как при repalceAll()) нужно использовать глобальные регулярные выражения (флаг g):
...
console.log(stock.replace(/(\d+) (\p{L}+)/gu, minusOne)); // no lemons, 1 cabbage and 100 eggs
```

где, параметры функции `minusOne`:

- `match` - текущее совпадение
- `ammount` - первая группа `(\d+)`
- `unit` - вторая группа `\p{L}+`
...
`+?`, `*?`, `??`, `{}?` - **ленивые** операторы получились из жадных путом дабавления `?`, они пытаются захватить минимальное количество символов.
...
`[^]*` - любой не пустой символ, включая перевод строки `\n` и каретки `\r`. В данном случае нельзя использовать просто точку `.`.
Точку можно использовать с флагом **s** `/./s`, который был добавлен в ES2018, включающий тотальный режим и точка теперь подразумевает все символы, включая новой строки и перевод каретки.

Оператор `*` жадный, поэтому в поледний вовод `1 1` не имеет плюса между единицами. Т.к. при откате назад на 4 символа патерн уже совпадает с объявленным.

Если после жадных операторов поместить `?` - то будет **ленивый** оператор, то есть он будет захватывать минимальное количество символов. Поэтому код должен быть:
...
Предпочтительно использовать ленивые операторы по умолчанию, чтобы избежать подобных проблем.

### Динамические регулярные выражения

Регулярные выражения, которые могут быть созданы во время выполнения программы.

Предположим хотим узнать есть ли заданное имя в тексте:
...
Свойство `source` содержит исходную строку регулярного выражения.

```js
let regexp = /a*/;
console.log(regexp.source); // → a*
```

#### RegExp.index

Свойство `index` содержит **индекс** первого совпадения при использовании флага `y` или `g`.

```js
let patern = /y/y.lastIndex = 1;
let match = patern.exec("xyzzy");
console.log(match.index); // → 1
```

#### RegExp.lastIndex

Свойство `lastIndex` содержит **индекс следующего символа** (массива или строки) с которого продолжится поиск дальнеиших совпадений при на личии флага `g` или `y`.

```js
let patern = /y/g;
patern.lastIndex = 3; // выставляем индекс для начала следующего поиска совпадений 3
let match = patern.exec("xyzzy"); // поиск начинается с индекса 3
console.log(match.index); // → 4 это индекс совпадения
console.log(patern.lastIndex); // → 5 следтоваетльно идндекс ледущющего поиска
```

При использовании флага `y` свойство `lastIndex` приклеивается к заданному ранее значению, т.е. к 3 и не перемещается.

Если совпадение не было найдено вообще, то свойство `lastIndex` сбрасывается на  `0`.

При переиспользовании регулярного выражения еще раз `lastIndex` сохраняется, что може привести к непредсказуемому поведению.

```js

let digit = /\d/g; // ← переиспользуемое регулярное выражение
console.log(digit.exec("here it is: 1")); // первое использование, где индекс следующего поиска выставлен в 13
console.log(digit.exec("and now: 1"));     // второе использование, начинает с 13 и т.к. индекс за пределами данной короткой строки
// то ничего не найдено и возвращается возвращает null
```
...
Строковый метод, аналогичный методу `exec()`, но имеет важное отличие если используется флаг `g` - вместо того, чтобы выводить классический массиво-подобный объект, возвращается массив со всеми найденным совпадениями:
...
Данный метод обязан иметь флаг `g`, иначе будет ошибка `matchAll called with a non-global RegExp argument`.

#### match и флаг `g`

Использовать когда нужно найти все вхождения

1. Безопасно:
   - с методом .replace()
   - при явном использовании свойства lastIndex, для лучшего контроля
2. Опасно:
   - с методом .math()

```js
const result = "Banana".match(/an/g); // → ["an", "an"]
```

Т.к. мы теряем информацию о позиции в тексте искомого шаблона, чтобы получить местоположение каждого совпадения (в массиве массивов) нужно использовать `String.matchAll()`.
...
В начале программисты оборачивали весь код модуля в функцию, которая сразу же испольняется (IIFE):
...
Хранилища не относятся к ECMAScript, но мы можем использовать их в JS.

### Куки

Cookie - строки с данными, хранящимися в браузере. Определены в стандарте протокола HTTP RFC6265.

Куки устанавливаются в память браузера веб-сервером с помощью HTTP-заголовка Set-Cookie.
Браузер же отправляет куки веб-серверу с помощью HTTP-заголовка Cookie, в основном такой обмен нужен для авторизации, тогда куки содержат уникальный для клиента TOKEN.

Установленные в браузер куки можно изменять с помощью свойства глобального объекта `window.document.cookie`.
Куки имеют формат ключ=значение, при изменении значения одного ключа, остальные остаются неизменными.
...
Создать куки можно в браузере с помощью javascript функции `document.cookie`, но в отличие от сервера с ограничениями:

1. Атрибут `HttpOnly`. Куки с этим флагом нельзя создать или изменить через JavaScript. Они устанавливаются только сервером через заголовок Set-Cookie. Такие куки автоматически отправляются на сервер, но недоступны для клиентского кода.
2. `Domain, Path`. Куки доступны только для данных домена/поддоменов. С помощью JS на другой домен куки не отправятся.
3. Флаг `Secure`. Куки с этим атрибутом передаются только по HTTPS. Если создать их через JS без Secure, они не будут отправлены на защищённый сервер.
4. Атрибут `SameSite`. Ограничивает отправку куки при межсайтовых запросах. Например, SameSite=Strict запрещает отправку при переходе с другого сайта.

Выгрузить куки из браузера в файл:

```js
const cookieData = document.cookie;const blob = new Blob([cookieData], { type: 'text/plain' });
...
Максимальный размер одного куки (ключ+значение) составляет 4 КБ.
Минимальное количество куки хранимых в браузере для одного домена более 20 в зависимости от браузера:

<table border="1" cellpadding="5" cellspacing="0">
  <caption>Практические лимиты в популярных браузерах</caption>
  <thead>
    <tr>
      <th>Браузер</th>
      <th>Макс. число куки на домен</th>
      <th>Лимит объёма на домен</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>Google Chrome</td>
      <td>до 176</td>
      <td>нет жёсткого лимита</td>
    </tr>
    <tr>
      <td>Safari</td>
      <td>до 242</td>
      <td>4 096 байт (4 КБ)</td>
    </tr>
    <tr>
      <td>Firefox</td>
      <td>до 50</td>
    ...
    <tr>
      <td>Internet Explorer (старые версии)</td>
      <td>до 20</td>
      <td>4 096–10 234 байт</td>
    </tr>
  </tbody>
</table>
...
Класс Storage является интерфейсом, предоставляющим доступ к хранилищу браузера имеет два подкласса:

- LocalStorage
- SessionStorage

Экземпляры подклассовlocalStorage и localStorage встроенны в среду браузера. Они хранят информацию только в браузере от 5МБ на домен (источник), объем можно расширить в настройках. Являются глобальными объектами в составе Window.

Данные представлены как ключ (строка) и значение (строка).

LocalStorage не удаляется при закрытии браузера, до их удаления пользователем или событием из кода clear().
SessionStorage доступен только из конкретной владки браузера и удаляются при закрытии вкладки или бразуера.

#### Методы storage

- setItem(key, value) - добавляет ключ и значение в хранилище
- removeItem(key) - удаляет ключ из хранилища
- getItem(key) - возвращает значение по ключу
- key(index) - возвращает ключ по индексу
- length - возвращает количество ключей
...
По объекту класса Storage можно итерироваться:
...
```console
Ключ: _ym105281677:0_reqNum, Значение: 3 debugger eval code:4:11
Ключ: _ym_retryReqs, Значение: {} debugger eval code:4:11
Ключ: _ym_wv2rf:105281677:0, Значение: 1 debugger eval code:4:11
Ключ: _ym105281677_lsid, Значение: 108621618838 debugger eval code:4:11
Ключ: _ym105281677_lastHit, Значение: 1763494570990 debugger eval code:4:11
Ключ: _ym_uid, Значение: "1763493476549602959" debugger eval code:4:11
undefined
```

## DOM

DOM (Document Object Model) - это спецификация, которая определяет структуру и поведение структуры дерева документа в браузере.

Это интерфейс, который позволяет парсит HTML или XML документ в DOM дерево, где каждый узел это объект.

Узел (node) - это базовая единица DOM (кнопка, атрибут, текст, комментарии и документ целиком).
Элемент - это узел с типом element node, т.е. не тождественно понятию узел.

Все узлы подразделяются по типам

- document node - корневой узел документа, содержащий остальные узлы
- document type node - тип документа, версия HTML или XML
- attribute node - атрибут (id, class, name, etc.) - сейчас не рассматривается как отдельный тип
- element node - html элементы (`<h1>, <p>`)
- text node - текстовый узел
- comment node - это комментарии

### Свойства узлов

#### Навигация по узлам

В консоли браузера можно обратиться к DOM дереву напрямую:

```js
document
```

Что выведет корневой объектвсе со всеми узлами документа от корня и другую информацию:
...
Данный объект будет содержать всю информацию о документе. Можно обратиться к корневому элементу напрямую:

```js
document.documentElement
```

Выведет только элемент html

Глубже к уникальным элементам внутри html можно обратиться напрямую только к двум:
...
Эти два свойства являются "первичными компонентами документа" (Primary document components), которые вынесены в отдельные объекты, для быстроты доступа. Они обладают как свойствами элементов, так и свойствами узлов. Глубже них по имени элемента напрямую не обратиться. Нарпмер, document.body.div - не работает.
Для обращения вглубь оснонвых компонентов можно использовать следующие свойства:

```js
document.body.firstElementChild // получает первый узел - это текст типа \n
document.head.lastElementChild // последний узел - это текст типа \n
```

Т.к. мы внутри body, который содержит большое количество элементов и узлов, то мы видим перенос строки в конце и начеле. Но, мы можем, также перемещаться по элементам и узлам, которые являются собратьями текстовых узлов, т.е. находяться на одном уровне вложенности:

```js
document.body.lastChild.previousSibling // отобразить некий div
document.head.firstChild.nextSibling // отобразитьекий title
```

Можно из элемента или узла обращаться к родительскому узлу, на уровено выше:
...
Если нужно отобразить все узлы внутри элемента:

```js
document.body.childNodes // NodeList(3) [text, div.wrapper, text]
```

Отобразиться коллекция узлов (псевдомассив). Их нельзя итеррировать методами массива (map, sort, reduce), но можно использовать циклы (for, for...of).

DOM-коллекции динамические (живые) и статические. При изменении документа, живая коллекция будет меняться. Коллекция доступна только для чтения

При получении коллекции узлов имеем много типов (текст, комментарии), которые особенно не нужны, но можно выводить HTML-коллекции, содержащую только элементы:

```js
document.body.lastElementChild.children
```

Т.е. можно обращаться как к узлам, так и к элементам напрямую.
...
document.body.firstElementChild // получает первый узел - это текст типа \n
document.head.lastElementChild // последний узел - это текст типа \n
document.body.previousSibling // отобразить head
document.head.nextSibling // отобразить body
...
```js
document.getElementByName('header') // возвращает живую коллекцию header - для форм и их кнопок
document.getElementsByTagName('p') // найти элемент по тегу
document.getElementById('id')
document.getElementsByClassName('class') // CSS класс
document.querySelectorAll('div > .class') // возвращает статическую коллекцию по селектору CSS
```

Статическая коллекция, в отличие от живой не меняется при обновлении DOM-дерева

Как правило нужно искать не все элементы по селлекторам, а только один (первый)
