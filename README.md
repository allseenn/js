...
Для того, чтобы функция принимала неограниченное количество параметров используют **rest-параметр**:
...
В данном случае все параметры собираются в массив.

#### spread-параметр

Наоборот распаковывает массив (объект) или другие структуры данных:
...
Можно передавать нераспакованный массив:
...
Проблема в том, что невозможно создать словарь (функции), на все случаи жизни. Но, можно использовать абстрактный подход - **фукнции высшего порядка**:

Можно написать конеретный цикл для повторения канкретного действия, например для вывода эелементов в консколь:

```js
for (let i=0; i<10; i++) console.log(i);
```

Если нужно данное действие выполняь желаемое количество раз (n), можно поместить цикл в фукнцию:
...
Функция `noisy` принимает другую функцию нахождения минимума в качестве аргумента и возвращает с помощью стрелочной конструкции "болтливую" версию, которая оповещает о начале и конце вызова. Для передачи аргументов болтливой фукнции используется цепочный вывоз:
