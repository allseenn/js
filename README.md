...
Встроенные функции и методы могут быть только синхронными или асинхронными, но есть в двух исполнениях сразу.
...
4. после освобождения стека колбеки перемещаются из очередей в него на выполененние
...
3. коллбеки асинхронных функций помещаются в очереди специализированных движков или процессоров:
    - **microtask queue**
...
Каждый отедльно взятый движок или процессор (JS, DOM, CSS) выполняет свои задачи синхронно в своих потоках, за общей асинхронностью следит цикл событий.
...
Уровень вложенности setTimeout() и других асихнронных фукнций может быть горомадным для описания асинхронной программы. Для того чтобы упростить написание кода и ускорить процесс его выполнения, была добавлена возможность записывать вышеприведенный код более лаконично с помощью промисов.
...
В прмерах ранеее в then почти не искользовали второй коллбек-обработчик `onRejected`, т.к. в новом синтаксите вместо него можно использовать метод catch:
