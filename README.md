...
Асинхронная операция - операция, которая не блокирует текущее выполнение коди и после запуска этой операции выполнение других задач продолжается, возвращение к медленной операции происходит тогда когда она веполенна, с помощью колбэков (обратных вызовов), это функции, которые будут вызваны после завершения медленной операции.

### Асинхронность в браузере

БРАУЗЕР
    ├── V8 Engine (ДВИЖОК JS) ← НЕ среда!
    │       └── Парсер
    │       └── Интерпретатор
    │       └── Компилятор
    │       └── Сall stack
...
Интерпретатор JS (interpreter) движка (см. [Введение](#intro)) при чтении кода разпределяет операции:

1. синхронные в call stack
2. асинхронные в среду, которая в своею очередь распределяет их:
    - callback queue
    - microtask queue

После выполеннения асинхронной задачи средой колбеки с помощью event loop в нужное время перемещаются из очередей в стэк вызовов где их обрабатывет сам движок.
...
3. API среды  (Web API, DOM API, Storage API, Network API, WebSockets API, Crypto API, Media API, Geolocation API)
   - Callback Queue - **очередь отложенных задач** для асинхронного кода
...
Вторая строка setTimeout() это асинхронная фукнция API браузера попадающя в Callback Queue, после выполенения это асинхронной фукнции через 5 секунд ее каллбек timeout() помещается в Call Stack, с пощью Event Loop.
Пока длятся 5 секунд, код продолжает выполнятся дальше выведя сообщение из 5 строки.
...
При занятом медленной синхронной операцией и следовательно забытым ей стеке нет гарантии, что функция timeout() выполнится в течении 5 секунд.

Асинхронными делают функции, которые чем-то могут быть заняты и не заблокируют основной поток, например:
...
Конструктор промиса принимает функцию-исполнитель, являющаюся колбэком, вызывается в момент создания промиса. Функция-исполнтиель принимает в качестве аргументов другие два коллбэка: resolve и reject:

- resolve - выполняется в случае успеха, возвращаюет результат value
- reject - выполняется в случае ошибки, возвращаюет результат error

Объекты Promise имеют свойства:

1. state - состояние промиса с одним из трех значений
    - pending (ожидание)
    - fulfilled (выполнено)
    - rejected (отклонено)
2. result - может иметь одно из трех значений
    - undefined (по умолчанию)
    - value (результат resolve)
    - error (результат reject)

Создадим задержку с помощью функции таймаута:
...
В коде выше и аналогичных возможны три вараинта при завершении прописа:

- выполенение колбека resolve
- выполнение колбека reject
- underfined отсутствие колбека (колбека не передано, состояние pending, ошибка в синтаксисе)
...
Самый частоиспользуемый метод в промисах, имеет синтаксис:

```js
promise.then(onFulfilled, onRejected)
```

then принимает два коллбека-обработчика, которые могут быть переданы по одному, чаще используют только первый, выполняется только один из двух:

1. onFulfilled выполняется если Promise.state == fulfilled
2. onRejected выполняется если Promise.state == rejected

Колбеки-обработчики нужны для того, чтобы совершить действие по резуллтату промиса. Данной действие нельзя выполнить в синхронной операции.

Коллбеки-обработчики принимает только по одному параметру, поэтому можно не указывать аргументы в скобках:
...
Если параметров then два, то можно записать так:
...
Вополнится только один исполнитель, в зависимости от состояния промиса. Т.е. console.log это функция, то она сама может быть передана как коллбек-обработчик:

```js
new Promise(resolve => resolve(1)).then(console.log);
```

Если коллбек-обработчик возвращает также объект промис, то для него можно вызвать then и так по цепочке:
