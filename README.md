...
Два опциональных обработчка яляются коллбеками (onFulfilled, onRejected) и могут быть переданы по одному, но чаще используют только первый.

В конце вызовется только один коллбек, в зависимости от состояния промиса:

- fulfilled - onFulfilled
- rejected - onRejected

Колбеки нужны для того, чтобы совершить действие по резуллтату промиса. Данной действие нельзя выполнтнить синхронно.

Если коллбек onFulfilled принимает один параметр, то можно не указывать его со скобками:
