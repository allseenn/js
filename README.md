...
Испоьзвется для объявления переменных с блочной областью видимости:
...
Рекомендуется имена свойств (ключи) объектов всегда заключать в ковычки, особенно:
...
Различие между ними заключается в том, что в точечной нотации свойство это строка, а в скобках это может быть вычисляемое выражение.
...
Для полного клонирования объъкта:
...
Встроенные (стандартные) методы, принимающие предикат, т.е. функцию возвращающую true либо false.

Большинство стандартных методов, кроме reduce, может принимать 3 аргумента:
...
Можно использовать деструктуризацию значений из объектов:
...
```js
let {name} = {name: "Slava", age: 21}
console.log(name); // Slava
```

При попытке деструктуризации null или undefined, будет ошибка.

Удобно пользоваться деструктуризацией параметров функции:
...
Это независимые от реализации интерфейсы
...
Разные куски взаимодействуют между собой с помощью **интерфейсов**

#### Интерфейсы

Это ограниченный набор **публичных** функций (методов) или переменных (свойств), которые предоставляют полезную функциональность на более абстрактном уровне, пряча тем самым их точную реализацию.
...
Для работы с прототипами можно использовать 2 метода: getPrototypeOf и setPrototypeOf.
...
Полиморфизм в JS основан на утиной типизации (duck typing): Если, что-то похоже на утку и крякает, то это утка.
