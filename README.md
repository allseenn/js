...
(04.Lecture)
Интерперетатор JS движка распредеяет выполененение операций по имени и направляет их на выполенение в:

- движок JS.
- среда (WEB API).

По приоритетам выполеннения:

1. в движке JS выполняются синхронные операции. Когда нужно где-то сохранить контекст функции, т.е. ее внутренную область видисомсти с переменными и объектами внутри него используют стек вызовов (call stack), т.е. функция которая последняя в цепочке рекурсий или во вложенности вызовов выполняется первой (LIFO).
2. задачи промисов и queueMicrotask попадают в микро очередь движка JS
3. В В веб среду в основном направляются асинхронные операции, где они распределяются по макро очередям конкретного движка среды.

Синхронные задачи помещаются в стек движка JS, асинхронные в очереди среды.
