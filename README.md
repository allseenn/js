...
Асинхронными сделаны функции, которые чем-то могут быть заняты они потенциально могут заблокировать основной поток, например:
...
Встроенные функции и методы могут быть только синхронными или асинхронными, а есть в двух исполнениях сразу.

Циклы for, while - является синхронным. Но, есть способ сделать код с такими операторами асинхронным

Интерпретатор JS различает синхронные и асинхронная функции, которые имеют свой поток:
...
1. асинхронная функция попадет в стек, где выполняется ее синхронная часть, например создается объект промиса
...
5. Асинхронная функция уйдя в API не блокирет поток, поэтому в стек попадает 5 строка с console.log
6. Console.log исполняется выводя "End synchronous code."
7. Находясь в WEB API setTimeout() ожидает 5 секунд
8. Через 5 секунд setTimeout() исполняется и возвращает в макротаск очередь свой коллбек console.log
9. Event Loop ожидает освобождение стека
...
Напрмер съемулируем получение данных сервера с задержками:
...
Конструктор промиса принимает функцию-исполнитель (executor), являющаюся колбэком, вызывается в момент создания промиса. Функция-исполнтиель принимает в качестве аргументов другие две внутренни функции менеющие состояние промиса:
...
При определении функции-исполнителя в ее тело можно записать любой код включай синхронный, например для вывода сообщени, но не рекомендуется, т.к. тяжелый код может подвесить event loop.
...
Данный метод позволяет выполнить код в конце цепочки промисов, независимо от состояния промиса.
...
Возвращает массив с результатами работы всех промисов, не важно они завершены успешно или нет.
