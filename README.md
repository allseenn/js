...
Асинхронная операция - операция, которая не блокирует текущее выполнение коди и после запуска этой операции выполнение других задач продолжается.

Асинхронный код состоит из двух частей:

- медленная операция (требует большего времени выполнения)
- callback - вызывается по завершению долгого кода, может сообщать о его завершении

**Цикл Событий** (Event Loop) - механизм, который помещает медленную операцию в очередь отложенных задач и следит за ними.
